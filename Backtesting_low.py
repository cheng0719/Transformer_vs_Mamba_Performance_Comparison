...
def Backtesting():
    tsmc_data = pd.read_csv('./Datasets/tsmc_stock_prices_INT_low_only_datetime_backtesting.csv')
    ...
    input_length = 30
    output_length = 1
    dataset = tsmc_data['low'].to_numpy()
    ...
    transformerModel.load_weights('./Weights/transformer_low_checkpoint.weights.h5')
    mambaModel.load_weights('./Weights/mamba_low_checkpoint.weights.h5')
...
# print('TSMC 2024/2/23 Low Price Prediction: ' + str(y_real[-1]))
# print('Transformer Predicted Low Price: ' + str(transformer_pred[-1]))
# print('Mamba Predicted Low Price: ' + str(mamba_pred[-1]))
